...
const newContent = [
  {id: '1', title: '#v-pop', image: require('../assets/images/sontung.jpg')},
  {id: '2', title: '#rock việt', image: require('../assets/images/sontung.jpg')},
  {id: '3', title: '#dopamine', image: require('../assets/images/sontung.jpg')},
];
...
const SearchScreen = () => {
  ...
  useEffect(() => {
    const fetchGenres = async () => {
      try {
        const response = await api.get('http://your-api-url.com/api/genre');
        // Ensure response has correct format
        const formatted = response.data.map(item: any => ({
          id: item.genreId.toString(),
          title: item.genreName,
          color: '#0D223A', // default or random
          image: require('../assets/images/sontung.jpg'), // fallback
        }));
        setGenres(formatted);
      } catch (err) {
        console.error('API lỗi, dùng mock:', err.message);
        setGenres(mockCategories);
      }
    };
