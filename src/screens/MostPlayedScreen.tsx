...
type SongInfoRouteProp = RouteProp<RootStackParamList, 'MostPlayed'>;

const MostPlayedScreen: React.FC = () => {
  ...
  console.log('albumId:', songId, 'Kiểu:', typeof songId);
  ...
  const fetchTracks = async () => {
    ...
    try {
      ...
      if (data?.length) {
        ...
        const mappedTracks = data.map((song: MostSong) => ({
          track: {
            name: song.title,
            id: song.songId,
  ...
  return (
    <TrackListScreen
      title="Bài hát được phát nhiều nhất"
      tracks={tracks}
